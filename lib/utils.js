...
// and optionally a value that's been passed too
// (ie. `redirect(<url>),<data>` )
const checkRedirect = function(formdata) {
  // this has added support for checkboxes, see below
  if (Object.keys(formdata).length !== 0) {
    for (var key in formdata) {
      if (typeof formdata[key] === 'string') {
        ...
        // this allows for redirects on checkboxes
        // will only support one per group (the last one found will be used)
        var r = null
        var newKey = ''
        formdata[key].forEach(function(item, i) {
          var match = item.match(/([^,]*).*redirect\(([^)]*)\)?/)
          if (match !== null) {
            // update the data for saving to session
            formdata[key][i] = match[1] || match[2]
            // return the redirect destination url
            r = match[2]
          }
        })
